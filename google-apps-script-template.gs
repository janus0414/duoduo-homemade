 ...
 * ⚠️ 此文件已停用 - 不再使用
 * 
 * 本專案已於 2026年3月5日 遷移至完全本地化的數據管理方案。
 * 所有網站內容現此由 site-data.js 統一維護，不再依賴 Google Sheet 與 Apps Script。
 * 
 * 文件保留作為歷史參考備份。如需恢復遠端數據源，請參考 git 歷史記錄或聯繫開發團隊。
 * 
 * 新的內容編輯流程：請直接編輯 site-data.js
 */
